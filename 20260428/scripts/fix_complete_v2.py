...
Uses firm_year_panel_final.csv as the source for additional variables.
"""
...
# Current panel (already has fiscal year fix, but missing some vars)
...
# Final panel with ALL variables (but old fiscal year alignment)
final_panel = pd.read_csv('/home/ricoz/econ_lab/FE-NLP/20260426/firm_year_panel_final.csv')
print(f"Final panel: {len(final_panel)} rows, {len(final_panel.columns)} cols")
...
# Variables that are in final_panel but need to be transferred
# These are the additional variables computed from WRDS
additional_vars = [
...
# Check what's available in final_panel
print("Additional vars in final panel:")
for v in additional_vars + ibes_vars:
    if v in final_panel.columns:
        non_null = final_panel[v].notna().sum()
        print(f"  {v}: {non_null}/{len(final_panel)} ({non_null/len(final_panel)*100:.1f}%)")
    else:
        print(f"  {v}: NOT FOUND")
...
# ---------------------------------------------------------------------------
# 4. Add additional variables from final_panel
# ---------------------------------------------------------------------------
print("\n=== Adding additional variables from final_panel ===")

# Create merge key: ticker + fiscal_year (NLP's original filing year) + accession
final_panel['tic_merge'] = final_panel['ticker'].replace(ticker_map).str.upper()
merged['tic_merge'] = merged['ticker'].replace(ticker_map).str.upper()

# Variables to bring from final_panel
vars_from_final = [v for v in (additional_vars + ibes_vars) if v in final_panel.columns]
print(f"Variables to transfer from final panel: {len(vars_from_final)}")

final_subset = final_panel[['tic_merge', 'fiscal_year', 'accession_number'] + vars_from_final].copy()
...
# Drop duplicate columns that came from final_panel (keep the new ones if they exist)
for col in list(merged.columns):
    if col.endswith('_final'):
        base_col = col[:-6]
        if base_col in merged.columns:
            # Keep final version where new version is null
