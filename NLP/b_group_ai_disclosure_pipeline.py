...
# Kimi CLI fallback (for coding-agent keys that block raw API calls)
USE_KIMI_CLI = os.getenv("USE_KIMI_CLI", "").strip().lower() in ("1", "true", "yes")
if USE_KIMI_CLI:
    # Validate that the CLI is available
    try:
        _kimi_version = subprocess.run(
            ["kimi", "--version"],
            capture_output=True,
            text=True,
            check=True,
        ...
        print(f"[Config] Using Kimi CLI: {_kimi_version.stdout.strip()}")
    except (subprocess.CalledProcessError, FileNotFoundError) as e:
        raise RuntimeError(
            "USE_KIMI_CLI is set but 'kimi' CLI is not found or not working. "
            "Please install it (https://github.com/MoonshotAI/kimi-cli) and run 'kimi login'."
        ) from e

# Auto-switch to OpenRouter for Kimi models when using a coding-agent key
# (Skip if we're using the Kimi CLI, which handles auth itself)
_USE_OPENROUTER = os.getenv("USE_OPENROUTER", "").strip().lower() in ("1", "true", "yes")
if not USE_KIMI_CLI and (_USE_OPENROUTER or ("kimi" in MODEL_NAME.lower() and "kimi.com/coding" in BASE_URL)):
    _openrouter_key = os.getenv("OPENROUTER_API_KEY", "").strip()
    if _openrouter_key:
        API_KEY = _openrouter_key
        BASE_URL = os.getenv("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
        if "kimi" in MODEL_NAME.lower() and "/" not in MODEL_NAME:
            MODEL_NAME = "moonshotai/kimi-k2.6"
        print(f"[Config] Routed Kimi through OpenRouter: model={MODEL_NAME}, base_url={BASE_URL}")
    else:
        print("[Warning] USE_OPENROUTER is set but OPENROUTER_API_KEY is missing.")
...
def _validate_setup() -> None:
    ...
    if not USE_KIMI_CLI and not API_KEY:
        raise ValueError(
...
# Thread-local client to avoid sharing connections across threads
...
# ============================================================
# 5. OpenAI API call (robust, concurrent-safe)
# ============================================================
